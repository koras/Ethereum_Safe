...
contract Safe{
    ...
    uint256 end;
    uint256 timeOutAuthentication;
    
    // arrays of safe keys
    mapping (address => bool) safeKeys;
    address [] massSafeKeys = new address[](4);
    
    // array of keys that signed the transaction
    mapping (address => bool) signKeys;
    ...
    function editCell(address _key, uint256 _lockup) secondLevel public returns(bool){
        ...
        /**Функция перемещения токенов из свободной суммы в ячейку. 
            Вход: Адрес ключа, управляющего ячейкой, количество токенов 
            Выход: Количество токенов при успехе или код ошибки при неудаче 
         */
    ...
    /***Функция вывода токенов из ячейки. 
        Вход: Количество токенов 
        Выход: При успехе - количество токенов принятых в сейф, если неудача, то код ошибки 
        */
    ...
    /**Функция перевода токенов из одной ячейки в другую. 
        Вход: Адрес ключа получателя, управляющего ячейкой и количество токенов 
        Выход: При успехе - количество токенов принятых в сейф, если неудача, то код ошибки 
        */
    ...
    function getMainBalance() public returns(uint256){
    ...
    */
    ...
    function getlastSafeKey() view public returns(address){
        return lastSafeKey;
    }
